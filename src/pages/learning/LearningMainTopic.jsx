...
export default function LearningMainTopic() {
  ...
  const handleAddMain = async () => {
    ...
    try {
      ...
      await axios.post("/learningMainTopic", formData, {
  ...
  const handleDeleteMain = async (id) => {
    ...
    try {
      await axios.delete(`/learningMainTopic/${id}`);
      fetchMainTopics();
    } catch (err) {
...
const handleToggleHide = async (topicId, isHidden) => {
  try {
    await axios.put(`/learningMainTopic/${topicId}/hide`, { isHidden });
  ...
  const handleSaveEdit = async (topic) => {
    try {
      ...
      await axios.put(`/learningMainTopic/${topic._id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">📘 المحاور الرئيسية للتعلم</h2>
