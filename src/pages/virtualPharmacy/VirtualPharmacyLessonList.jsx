  ...
  const fetchLectureAndLessons = async () => {
    try {
      ...
      const [lecRes, lesRes] = await Promise.all([
        axios.get(`/virtualPharmacyMainTopic/${mainId}`),
        axios.get(`/VirtualPharmacyLesson/byLecture/${mainId}`),
      ]);
...
const handleToggleHide = async (lessonId, isHidden) => {
  try {
    await axios.put(`/VirtualPharmacyLesson/${lessonId}/hide`, { isHidden });
  ...
  const handleAddLesson = async () => {
    ...
    try {
      ...
      formData.append("topicId", mainId);
  ...
  const handleDeleteLesson = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`/VirtualPharmacyLesson/${id}`);
        fetchLectureAndLessons();
