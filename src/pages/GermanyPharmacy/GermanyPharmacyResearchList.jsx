...
export default function GermanyPharmacyResearchList() {
  ...
  const fetchMainAndResearches = async () => {
    try {
      ...
      const [mainRes, researchRes] = await Promise.all([
        axios.get(`/germanyPharmacyMainTopic/${mainId}`),
        axios.get(`/germanyPharmacyResearch/byMain/${mainId}`)
      ]);
  ...
  const handleAddResearch = async () => {
    ...
    try {
      await axios.post(`/germanyPharmacyResearch`, { 
        name: newResearchAr, 
  ...
  const handleUpdateResearch = async (id) => {
    ...
    try {
      await axios.put(`/germanyPharmacyResearch/${id}`, {
...
const handleToggleHide = async (researchId, isHidden) => {
  try {
    await axios.put(`/germanyPharmacyResearch/${researchId}/hide`, { isHidden });
  ...
  const handleDeleteResearch = async (id) => {
    ...
    try {
      await axios.delete(`/germanyPharmacyResearch/${id}`);
      fetchMainAndResearches();
    } catch (err) {
              ...
              <tr {...dragProps} className="border-t hover:bg-gray-50">
                ...
                <td className="p-2 text-center">
                  <div className="flex justify-center gap-2">
                    {/* 👁 عرض تفاصيل البحث */}
                    <button
 onClick={() => navigate(`/germanyPharmacyresearch/${r._id}/lectures`,{
      state: {
        mainId: mainId,
