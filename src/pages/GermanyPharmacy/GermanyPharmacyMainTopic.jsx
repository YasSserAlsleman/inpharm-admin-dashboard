...
export default function GermanyPharmacyMainTopic() {
  ...
  const handleAddMain = async () => {
    ...
    try {
      ...
      await axios.post("/germanyPharmacyMainTopic", formData, {
  ...
  const handleDeleteMain = async (id) => {
    if (!window.confirm(t('germanyPharmacy.deleteConfirm', 'هل تريد حذف هذا المحور الرئيسي؟'))) return;
    try {
      await axios.delete(`/germanyPharmacyMainTopic/${id}`);
      fetchMainTopics();
    } catch (err) {
...
const handleToggleHide = async (topicId, isHidden) => {
  try {
    await axios.put(`/germanyPharmacyMainTopic/${topicId}/hide`, { isHidden });
  ...
  const handleSaveEdit = async (topic) => {
    try {
      ...
      await axios.put(`/germanyPharmacyMainTopic/${topic._id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
...
<div className="bg-white p-6 rounded-lg shadow-md mb-6 max-w-lg mx-auto flex flex-col gap-4">
  ...
  <div className="grid grid-cols-1 gap-3">
    <input
      ...
      placeholder={t('germanyPharmacy.nameAr', 'اسم المحور الرئيسي (العربية)')}
      value={newMainNameAr}
      onChange={(e) => setNewMainNameAr(e.target.value)}
    />
    <input
      className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
      placeholder={t('germanyPharmacy.nameEn', 'Main topic name (English)')}
      value={newMainNameEn}
      onChange={(e) => setNewMainNameEn(e.target.value)}
    />
    <input
      className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
      placeholder={t('germanyPharmacy.nameDe', 'Titel des Hauptthemas (Deutsch)')}
      value={newMainNameDe}
    ...
    <textarea
      ...
      placeholder={t('germanyPharmacy.descriptionAr', 'وصف المحور (العربية)')}
    ...
    <textarea
      className="border border-gray-300 rounded px-3 py-2 overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
      placeholder={t('germanyPharmacy.descriptionEn', 'Topic description (English)')}
    ...
    <textarea
      className="border border-gray-300 rounded px-3 py-2 overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
      placeholder={t('germanyPharmacy.descriptionDe', 'Beschreibung des Themas (Deutsch)')}
  ...
  >
    {t('germanyPharmacy.addTopic', 'إضافة محور')}
  </button>
</div>
